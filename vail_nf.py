...
from models.vail import Actor, Critic, Discriminator
...
class GAIL:
    def __init__(self, vail_sample, reward_shift, reward_aug, gae_norm, global_norm,
                actor_lr, critic_lr, disc_lr, actor_units, critic_units, disc_units, disc_reduce_units,
    ...
    def update(self):
        ...
        for i in range(batch_num):
            ...
            with tf.GradientTape(persistent=True) as tape:
                mu, std, sampled = self.discriminator.encode(self.encoder, s_concat, a_concat)

                discs = self.discriminator.decode(sampled if self.vail_sample else mu)
            ...
            self.discriminator.opt.apply_gradients(zip(disc_grads, self.disc_vars))
            del tape
        ...
        for _ in range(self.epochs):
            ...
            for i in range(batch_num):
                ...
                self.critic.opt.apply_gradients(zip(critic_grads, self.critic_vars))
                del tape
                ...
                actor_grads = tape.gradient(actor_loss, self.actor_vars)       # NOTE: freeze posterior
                if self.grad_global_norm > 0:
                    actor_grads, _ = tf.clip_by_global_norm(actor_grads, self.grad_global_norm)
                self.actor.opt.apply_gradients(zip(actor_grads, self.actor_vars))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--unload_enc', action='store_true')
    parser.add_argument('--render',     action='store_true')
    parser.add_argument('--verbose',    action='store_true')
    parser.add_argument('--gae_norm',   action='store_true')
    parser.add_argument('--vail_sample',action='store_true')
    ...
    parser.add_argument('--log_name',   type=str,   default='vail')
