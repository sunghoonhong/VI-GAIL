...
from models.vail_local import Actor, Critic, Discriminator
...
class GAIL:
    ...
    def dummy_forward(self):
        ...
        self.discriminator(dummy_state, dummy_action)
    ...
    def get_reward(self, states, actions):
        d = self.discriminator(states, actions).numpy()   # (N, 1)
    ...
    def get_gae_oracle(self, states, rewards, done):
        ...
        values = self.critic(states).numpy()   # (N+1, 1)
        if done:
    ...
    def update(self):
        ...
        for i in range(batch_num):
            ...
            with tf.GradientTape(persistent=True) as tape:
                mu, std, sampled = self.discriminator.encode(s_concat, a_concat)
            ...
            disc_grads = tape.gradient(discriminator_loss, self.discriminator.trainable_variables)
